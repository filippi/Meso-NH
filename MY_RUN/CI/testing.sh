...
if [ ${force} -eq 1 -o $(get_statuses "${SHA}" | grep "${context}" | wc -l) -eq 0 ]; then
  ...
  for model in $models; do
    ...
    #Commande
    cmd="/home/rodierq/GIT/MNH-CI/MNH-ladev/MY_RUN/CI/check_commit_${model}.sh --repo-user ${MESONHREPOuser} --repo-protocol ${PHYEXREPOprotocol} ${SHA}"
